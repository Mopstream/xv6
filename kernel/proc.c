...
procinit(void) {
    ...
    initlock(&lst_lock, "lst");
    //initlock(&free_lock, "free_lst");

    proc_lst_init(&lst);
    //proc_lst_init(&free_proc);
}
...
freeproc(struct proc *p) {
    ...
    struct proc_list *l = (struct proc_list *) p;
    proc_lst_remove(l);
    bd_free(l->p);
    bd_free(l);
...
wait(uint64 addr) {
    ...
    for (;;) {
        ...
        sleep(p, &lst_lock);  //DOC: wait-sleep
    }
}
...
int dump(void) {
    uint64 * reg_ptr = &(myproc()->trapframe->s2);
