...
// Reverse bit at position index in array
void bit_reverse(char *array, int index) {
  if (bit_isset(array, index)) bit_clear(array, index);
  else bit_set(array, index);
}
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  can_merge:");
    bd_print_vector(bd_sizes[k].can_merge, NBLK(k));
  }
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_reverse(bd_sizes[k].can_merge, blk_index(k, p) / 2);
  ...
  for (; k > fk; k--) {
    ...
    bit_reverse(bd_sizes[k - 1].can_merge, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_reverse(bd_sizes[k].can_merge, bi/2);
    if (bit_isset(bd_sizes[k].can_merge, bi/2)) {
      break;
    }
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sz / 2 > 0 ? sz / 2 : 1;
    bd_sizes[k].can_merge = p;
    memset(bd_sizes[k].can_merge, 0, sz);
    p += sz;
  }
